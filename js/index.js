import{u as w,g as $,a as L,T as g,b as h,L as y,M as T,P as S,h as k,o as M,l as j}from"../assets/signOut-7ANRpM1j.js";/* empty css                     */import{g as O,a as x,c as D,S as v,G as E}from"../assets/vendor-CHXV-Q4N.js";function H(){document.addEventListener("click",e=>{t(e),s(e)});function t(e){var r;const i=e.target.closest(".header__user-inner-item"),a=e.target.closest(".header__user-item-box");!(i!=null&&i.classList.contains("header__user-inner-item"))&&!(a!=null&&a.classList.contains("header__user-item-box"))&&((r=w)==null||r.classList.remove("header__user-inner-list_open"))}function s(e){var r;const i=e.target.closest(".game-dropdown-item__inner-item"),a=e.target.closest(".js-game-dropdown-box");!(i!=null&&i.classList.contains("game-dropdown-item__inner-item"))&&!(a!=null&&a.classList.contains("js-game-dropdown-box"))&&((r=$)==null||r.classList.remove("game-dropdown-item__inner-list_open"))}}async function _(t){const s=O(L);try{const e=await x(D(s,t)),i=[];return e.forEach(a=>{i.push({id:a.id,...a.data()})}),i}catch(e){return console.error("Error getting documents:",e),[]}}function q(t){const s=document.querySelector(".js-main-slider-news");if(!s)return;const e=t.map(i=>{const{title:a,description:o,views:r,date:n,image:{png:c,webp:l},author:{img:d,name:p}}=i;return`
      <li class="splide__slide">
          ...
          </li>`});s.innerHTML=e.join("")}function P(t,s){if(!t.length){s.innerHTML='<li><p class="list-top__empty">No data available</p></li>';return}const e=[...t].sort((i,a)=>a.score-i.score).map(i=>{const{name:a,score:o,id:r,countryFlagImg:{webp:n},avatar:{png:c,webp:l}}=i;return`
           <li class="list-top__item" data-id="${r}">
            <p class="list-top__item-num"></p>
            <picture>
              <source
        ...
        <li class="list-cards__item" data-id="${a}">
          ...
          <div class="list-cards__bottom-content">
            ...
            </li>`});s.innerHTML=e.join("")}function W(t=" "){_(t).then(s=>U(s)).catch(s=>console.error("Error fetching data:",s))}function V(){G(T),W(S)}document.addEventListener("DOMContentLoaded",()=>{k(),H(),A(),C(),R(),z(),V(),M(),j()});
